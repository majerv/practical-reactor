...
public class c5_CreatingSequence {
    ...
    public void callable_counter() {
        ...
        Mono<Integer> callableCounterMono = Mono.create(sink -> {
            try {
    ...
    /**
     * Create Flux that emits incrementing numbers at interval of 1 second.
     */
    @Test
    ...
    public void generate_programmatically() {
        final var generateCounter = new AtomicInteger();

        Flux<Integer> generateFlux = Flux.generate(sink -> {
            int i = generateCounter.incrementAndGet();
            sink.next(i);
        ...
        Flux<Integer> createFlux = Flux.create(sink -> Flux.range(1, 5).doOnNext(sink::next).doOnComplete(sink::complete).subscribe());

        //------------------------------------------------------

        Flux<Integer> pushFlux = Flux.push(sink -> Flux.range(1, 5).doOnNext(sink::next).doOnComplete(sink::complete).subscribe());
